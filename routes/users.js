...
router.post( '/upload', auth,  async function(req, res, next) {
  ...
  form.on('fileBegin', (name, file) => {
    file.path = path.resolve(__dirname + './../uploads/' + userPhone + path.parse(file.name).ext );
    uploadFile.url = 'http://' + process.env.HOST + ':3001/uploads/'+ userPhone + path.parse(file.name).ext
    console.log(4);
  });
