...
const SendVerificationCode = async function (userName, userPhone) {
  return new Promise((resolve, reject) => {
    ...
    Pool.query(
      "DELETE FROM SMScodes WHERE mobileNumber = (?);",
      [mobileNumber],
      async (err, row, fields) => {
        Pool.query(
          "INSERT INTO SMScodes (mobileNumber, mobileCode, codeExpires, cusFirstNameToUpdate) VALUES ((?),(?),(?),(?));",
          [mobileNumber, mobileCode, codeExpires, cusFirstNameToUpdate],
          async (err, row, fields) => {
...
const sendSMS = async function (Phone, msg) {
  ...
  axios
    .post("http://gapi.soprano.co.il/QuickGateway.ashx?apiprotocol=json", {
      sms: {
        account: {
